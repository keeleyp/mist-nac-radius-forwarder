...
## Working around Start/Stop quirks: force_interim_update (v4.2+)

Some NAS/RADIUS-accounting-profile implementations refresh identity cleanly on `Interim-Update` but handle `Start`/`Stop` transitions differently. This is a separate, empirically-driven option from the `Class`/`Filter-Id` fix above — if your FortiGate seems to process accounting more reliably when every packet arrives as `Interim-Update`, set `force_interim_update = true` under `[attributes]` in `mist-radius.ini` or `mist-combined-radius.ini` to test that. It only changes the `Acct-Status-Type` value on the wire (always `3`, regardless of the underlying Mist event) — `Start`/`Stop`-specific attributes (`Acct-Session-Time`, `Acct-Terminate-Cause`) are still included exactly as before, and internal session-lifecycle tracking is unaffected. Off by default, since it deviates from RFC 2866's documented semantics — only enable it if your FortiGate/NAS is demonstrably behaving better with it on. `mist_nac_events_radius.py` only ever sends Start and has nothing to toggle. Guide sections 5.7 / 25.6.
